class node:
    ...
    def __init__(self, nodes, x,y,z,w, turn=0):
        self.nodes = nodes
        self.coord = tuple([x,y,z,w])
        self.turn = turn
        self.start = turn
        self.turns = []
        self.neighbors = []
...
class nodes:
    ...
    def gen_node(self, x,y,z,w, init=False):
        tup = tuple([x,y,z,w])
        if tup in self.nodes.keys():
            n = self.nodes[tup]
        else:
            n = node(self, x,y,z,w, self.turn)
        n.cur = init
        self.nodes[tup] = n
        if init:
            self.get_neigh(n)
    ...
    def z_level(self, z,w):
        kdic = {}
        keys = list(filter(lambda k: (k[2] == z) and (k[3] == w), self.nodes.keys()))
        for k in keys:
...
if __name__ == '__main__':
    ...
    for y in range(len(lines)):
        ...
        for x in range(len(chars)):
            if chars[x] == '#':
                N.gen_node(x,y,z,w,True)
    
    N.run_to(6)
    print(f"Day 17 Part 2: { N.count() }")